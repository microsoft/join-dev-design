  new Vue({
  	...
  	created() {
  		console.log("👋 Simon says check out the microsoft logo when the page loads ✅");
  	...
  	computed: {
  		...
  		logoTitleGreen: function () {
  			return this.colorStatus.green ? 'logo-tile--lightgreen' : 'logo-tile--green';
  		},
  		logoTitleYellow: function () {
  			return this.colorStatus.yellow ? 'logo-tile--lightyellow' : 'logo-tile--yellow';
  		},
  	...
  	methods: {
  		generateSequence: function () {
  			this.currentSequence.push(this.colors[this.getRandomInt(this.colors.length)]);
  			this.play();
  		},
  		clickColor: function (color) {
  			...
  			if (this.checkCorrect()) {
  				this.userClicks = [];
  				this.currentScore++;
  				console.log(`Simon says..."correct!" 🎉 Your score is`, this.currentScore);
